    ...
    <div class="flex-1 flex flex-col bg-white">
      <template v-if="active">
        ...
        <div class="px-4 py-3 border-t border-black/5 shrink-0 flex gap-2">
          ...
          <button @click="fileInput.click()" class="w-10 h-10 flex items-center justify-center text-lg hover:bg-gray-100 transition-colors cursor-pointer shrink-0" title="发送图片">🖼</button>
          <input
            v-model="text"
            @keydown.enter="send"
            placeholder="输入消息..."
            class="flex-1 px-4 py-2.5 bg-gray-50 border border-black/5 text-sm outline-none focus:border-[rgb(196,147,51)]"
          />
          <button
            @click="send"
            :disabled="!text.trim()"
...
try {
  const u = JSON.parse(localStorage.getItem('seller_user') || localStorage.getItem('user') || '{}');
  myUserId.value = u.id;
} catch {}
...
async function onFilePicked(e) {
  ...
  try {
    ...
    console.error('[ChatPage] 图片上传失败:', err);
  } finally {
...
onMounted(() => {
  ...
  socket.value = io(`${import.meta.env.PROD ? '' : 'http://localhost:3000'}/chat`, {
    auth: { token },
