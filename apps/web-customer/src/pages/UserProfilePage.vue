<template>
  <div class="bg-white min-h-screen">
    <!-- Tab bar -->
    <div
      class="sticky top-0 z-20 bg-white/95 backdrop-blur-md border-b border-gray-100"
    >
    ...
    <div class="max-w-[800px] mx-auto px-6 pb-20">
        ...
        <template v-else>
          ...
          <div class="pt-10 pb-8 flex flex-col items-center">
            <div class="relative group">
              <div
                class="w-24 h-24 rounded-full overflow-hidden bg-gray-100 flex items-center justify-center shadow-md"
              >
                <img
                  v-if="profile.avatar"
                  :src="coverSrc(profile.avatar)"
                  class="w-full h-full object-cover"
                />
                <span v-else class="text-3xl font-semibold text-gray-400">{{
            ...
            <p v-if="avatarError" class="text-red-400 text-xs mt-2">
              {{ avatarError }}
            </p>
          </div>
          ...
          <div class="mb-6">
            ...
            <p class="text-[15px] bg-gray-50 px-4 py-3">
              &yen;{{ profile.balance ?? 0 }}
            </p>
          </div>
          <div
            class="mb-6 p-5 border border-dashed border-[rgb(196,147,51)]/30 bg-[rgb(196,147,51)]/[0.02]"
          >
            ...
            <p
              ...
              :class="[
                'text-xs mt-2',
                rechargeOk ? 'text-green-600' : 'text-red-500',
              ]"
            >
              {{ rechargeMsg }}
            </p>
          ...
          <div class="mb-8 p-5 border border-black/5">
            ...
            <div v-if="showPwd" class="mt-4 space-y-3">
              ...
              <p
                v-if="pwdMsg"
                :class="['text-xs', pwdOk ? 'text-green-600' : 'text-red-500']"
              >
                {{ pwdMsg }}
              </p>
            </div>
          ...
          <div class="mb-8">
            <label class="block text-sm text-gray-500 mb-1.5"
              >默认收货地址 <span class="text-red-400 ml-0.5">*</span></label
            >
            <CitySelect v-model="address" v-model:detail="addressDetail" />
            <p v-if="addressError" class="text-red-400 text-sm mt-1.5">
              {{ addressError }}
            </p>
          </div>
          <!-- 隐私设置 -->
          ...
          <p v-if="saved" class="text-green-600 text-sm text-center mt-3">
            保存成功
          </p>
        </template>
      </div>
      ...
      <div v-if="activeTab === 'purchases'" class="pt-10">
        ...
        <template v-else-if="purchases.length === 0">
          <div class="text-center py-20 text-gray-400">
            ...
            <router-link
              to="/catalog"
              class="text-[rgb(196,147,51)] text-sm hover:underline mt-2 inline-block"
              >去逛逛 →</router-link
            >
          </div>
        </template>
        <template v-else>
          <div class="space-y-6">
            ...
            >
              <div
                class="shrink-0 w-[100px] h-[100px] overflow-hidden"
              ...
              <div class="flex-1 min-w-0">
                ...
                <p class="text-xs text-gray-400 mt-1">
                  {{ album.tracks?.length || 0 }} 首曲目
                </p>
                <div class="mt-3 space-y-1">
                  <button
                    v-for="track in (album.tracks || []).filter(
                      (t) => t.audioUrl,
                    )"
                    :key="track.id"
                  ...
                  >
                    ...
                    <span v-if="track.duration" class="text-gray-300 ml-auto">{{
                      track.duration
                    }}</span>
                  </button>
                </div>
      ...
      <div v-if="activeTab === 'favorites'" class="pt-10">
        <div v-if="favsLoading" class="text-center py-20 text-gray-400 text-sm">
          加载中...
        </div>
          ...
          <div class="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-4">
            ...
            >
              <div
                ...
                :style="{ background: fav.album.gradient || '#f3f4f6' }"
              >
                <img
                  v-if="fav.album.coverUrl"
                  :src="coverSrc(fav.album.coverUrl)"
                  class="w-full h-full object-cover"
                />
              </div>
              <div class="p-3">
                ...
                <button
                  ...
                  class="mt-2 w-full py-1.5 text-xs bg-gray-100 text-gray-400 cursor-default"
                >
                  已购买
                </button>
                <button
                  ...
                  class="mt-2 w-full py-1.5 text-xs bg-[rgb(196,147,51)] text-white hover:bg-[rgb(176,127,31)] transition-colors"
                >
                  购买后收听
                </button>
              </div>
      ...
      <div v-if="activeTab === 'history'" class="pt-10">
        ...
        <template v-else>
          <div class="space-y-1">
            <div
              v-for="(h, idx) in playHistory"
              :key="h.id"
              class="flex items-center gap-4 px-4 py-3 hover:bg-gray-50 transition-colors group"
            >
              <span class="text-xs text-gray-300 w-6 text-right shrink-0">{{
                playHistory.length - idx
              }}</span>
              <button
              ...
              <div
                class="w-10 h-10 shrink-0 overflow-hidden cursor-pointer"
                :style="{ background: h.album.gradient || '#f3f4f6' }"
                @click="$router.push(`/albums/${h.album.slug}`)"
              >
                <img
                  v-if="h.album.coverUrl"
                  :src="coverSrc(h.album.coverUrl)"
                  class="w-full h-full object-cover"
                />
              </div>
              <div
...
async function loadTabData() {
  ...
  if (activeTab.value === "history" && playHistory.value.length === 0) {
    ...
    try {
      playHistory.value = await fetchPlayHistory(30);
    } catch {
    } finally {
