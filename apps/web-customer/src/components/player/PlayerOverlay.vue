<template>
  <Transition name="overlay">
    <div v-if="player.showFullPlayer" class="fixed inset-0 z-[200] flex flex-col">
      <!-- 背景 -->
      <div class="absolute inset-0 transition-colors duration-700" :style="{ background: bgGradient }"></div>
      <div class="absolute inset-0 bg-black/50 backdrop-blur-md" @click="close"></div>

      <!-- 内容 -->
      <div class="relative z-10 flex flex-col h-full max-w-[1100px] mx-auto w-full px-6 py-6">
        ...
        <!-- 中间主体：唱片 + 信息 -->
        <div class="flex-1 flex items-center gap-10 min-h-0 max-lg:flex-col max-lg:justify-center max-lg:gap-6">
          <!-- 左侧：旋转唱片 -->
          <div class="shrink-0 flex items-center justify-center max-lg:max-w-[260px]">
            <div class="relative w-[min(340px,40vw)] h-[min(340px,40vw)] max-lg:w-[min(260px,60vw)] max-lg:h-[min(260px,60vw)]">
              <!-- 唱片底纹 -->
              <div
              ...
              >
                ...
                />
                <div v-else class="w-full h-full flex items-center justify-center bg-white/10 text-white/30 text-5xl">
                  ♫
                </div>
              </div>
              <!-- 中心小圆 -->
          ...
          <!-- 右侧：歌曲信息 -->
          <div class="flex-1 min-w-0 text-white max-lg:text-center max-lg:w-full">
            <h1 class="text-[clamp(22px,3vw,32px)] font-bold tracking-[-0.02em] leading-tight mb-1 truncate">
              {{ player.track?.title || '未知歌曲' }}
            </h1>
            <p class="text-[15px] text-white/50 mb-0.5">
              专辑：<span class="text-white/70">{{ player.album?.title || '未知专辑' }}</span>
            </p>
            <p class="text-[15px] text-white/50 mb-0.5">
              歌手：<span class="text-white/70">{{ player.track?.artist || player.album?.artist || '未知歌手' }}</span>
            </p>
            <p v-if="player.album?.categories?.length" class="text-[13px] text-white/40 mt-1">
              <span v-for="(cat, i) in player.album.categories" :key="cat.id || cat.slug || i">
                <span v-if="i > 0"> / </span>{{ cat.name || cat.slug }}
              </span>
            </p>

            <!-- 进度条 -->
            <div class="mt-6 flex items-center gap-3 max-w-[500px] max-lg:mx-auto">
              <span class="text-xs text-white/40 w-10 text-right">{{ formatTime(player.currentSeconds) }}</span>
              <div class="flex-1 relative h-1 group cursor-pointer" @click="seekBar">
                ...
                ></div>
                <div
                  class="absolute top-1/2 -translate-y-1/2 w-3 h-3 rounded-full bg-white opacity-0 group-hover:opacity-100 transition-opacity shadow-lg"
            ...
            <!-- 控制按钮 -->
            <div class="mt-5 flex items-center gap-5 max-lg:justify-center">
              <!-- 上一首（暂用占位） -->
              <button class="w-10 h-10 rounded-full bg-white/10 hover:bg-white/20 text-white/60 flex items-center justify-center transition-colors cursor-not-allowed" disabled>
                <svg xmlns="http://www.w3.org/2000/svg" class="w-5 h-5" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="2">
              ...
              <!-- 播放/暂停 -->
              <button @click="togglePlay" class="w-14 h-14 rounded-full bg-[rgb(196,147,51)] hover:bg-[rgb(176,127,31)] text-white flex items-center justify-center transition-all hover:scale-105 shadow-lg shadow-[rgb(196,147,51)]/30">
                <span v-if="!player.playing" class="text-2xl ml-0.5">▶</span>
        ...
        <div class="shrink-0 mt-6">
          ...
          <div class="flex gap-0 border-b border-white/10">
            <button
              ...
              class="px-6 py-2.5 text-[14px] font-medium transition-colors relative"
              :class="activeTab === tab.key ? 'text-white' : 'text-white/40 hover:text-white/70'"
            >
              {{ tab.label }}
              <div
                v-if="activeTab === tab.key"
                class="absolute bottom-0 left-1/2 -translate-x-1/2 w-8 h-[3px] rounded-full bg-[rgb(196,147,51)]"
              ></div>
            </button>
          </div>

          <!-- Tab 内容 -->
          <div class="py-5 max-h-[200px] overflow-y-auto custom-scrollbar">
            <!-- 百科 -->
              ...
              <div v-else class="grid grid-cols-4 gap-3 max-lg:grid-cols-3 max-sm:grid-cols-2">
                ...
                >
                  <div class="aspect-square rounded-lg overflow-hidden mb-1.5 shadow-md"
                    ...
                    />
                    <div v-else class="w-full h-full flex items-center justify-center text-white/20 text-2xl">♫</div>
                  </div>
...
const bgGradient = computed(() => {
  // 使用专辑的 gradient 或默认暗色
  if (player.album?.gradient) {
    // gradient 通常是 linear-gradient(...) 或纯色
